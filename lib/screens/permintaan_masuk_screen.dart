...
class _PermintaanMasukScreenState extends State<PermintaanMasukScreen> {
        ...
        builder: (ctx, setStateDialog) {
          return AlertDialog(
            ...
            content: SizedBox(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Pilihan alasan
                  ..._alasanList.map((alasan) => GestureDetector(
                        onTap: () => setStateDialog(() => _alasanDipilih = alasan),
                        child: Container(
                          width: double.infinity,
                          margin: const EdgeInsets.only(bottom: 6),
                          padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10),
                          decoration: BoxDecoration(
                            color: _alasanDipilih == alasan
                            ...
                            border: Border.all(
                              ...
                              width: _alasanDipilih == alasan ? 1.5 : 0.5,
                            ),
                          ),
                          child: Row(children: [
                            Icon(
                              _alasanDipilih == alasan
                            ...
                            Expanded(
                              child: Text(alasan,
                                  style: TextStyle(
                                      ...
                                      fontWeight: _alasanDipilih == alasan
                                          ? FontWeight.w500
                                          : FontWeight.normal)),
                            ),
                          ]),
                  ...
                  // Field alasan custom jika pilih "Lainnya"
            ...
            actions: [
              ...
              ElevatedButton(
                onPressed: _alasanDipilih == null
                    ? null
                    : () {
                        final alasanFinal = _alasanDipilih == 'Lainnya' && alasanCtrl.text.isNotEmpty
                            ? alasanCtrl.text
                            : _alasanDipilih!;
                        Navigator.pop(ctx);
                ...
                style: ElevatedButton.styleFrom(
                  backgroundColor: _alasanDipilih == null
                      ? AppColors.borderCard
                      : AppColors.textDanger,
  ...
  // ── Proses tolak → update status + kirim ke riwayat ────────────────────
  void _prosesTolak(_PermintaanItem item, String alasan) {
    setState(() {
      final idx = _items.indexWhere((e) => e.id == item.id);
      if (idx != -1) {
        _items[idx] = _items[idx].copyWith(status: 'Ditolak', alasanTolak: alasan);
      }
    });

    // Kirim ke TransaksiProvider (riwayat)
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Permintaan ditolak & masuk ke riwayat'),
        backgroundColor: AppColors.textDanger,
      ),
    );
  }

  // ── Proses setujui → kirim ke riwayat ──────────────────────────────────
  void _prosesSetujui(_PermintaanItem item) {
    ...
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(
        content: Text('Permintaan disetujui & masuk ke riwayat'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: buildAppBar('Permintaan Masuk', subtitle: '$menunggu permintaan menunggu'),
      body: ListView(
            ...
            child: AppCard(
              ...
              child: Column(
                ...
                children: [
                  ...
                  // Info rekening (hanya jika masih pending)
                  if (isPending)
                    Container(
                      width: double.infinity,
                  ...
                  // Alasan tolak (tampil jika ditolak)
                  ...
                  // Tombol tolak/setujui (jika belum ada bukti & masih pending)
                  if (isPending && !hasBukti)
                    Row(children: [
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        ...
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Center(child: Container(width: 36, height: 4, decoration: BoxDecoration(color: AppColors.borderCard, borderRadius: BorderRadius.circular(2)))),
          const SizedBox(height: 16),
          const Text('Bukti Pembayaran', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w600, color: AppColors.textPrimary)),
          const SizedBox(height: 2),
          ...
          Container(
            padding: const EdgeInsets.all(10),
            decoration: BoxDecoration(color: AppColors.bgPage, borderRadius: BorderRadius.circular(8), border: Border.all(color: AppColors.borderCard, width: 0.5)),
            child: Row(children: [
          ...
          const Text('Foto bukti transfer:', style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: AppColors.textMuted)),
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        const SizedBox(height: 8),
                        Text(widget.item.buktiPembayaran ?? '', style: const TextStyle(fontSize: 11, color: AppColors.textSecondary)),
                        const SizedBox(height: 4),
                        Text(_zoomBukti ? 'Tap untuk perkecil' : 'Tap untuk perbesar', style: const TextStyle(fontSize: 10, color: AppColors.chipDefaultText)),
                      ],
                    ),
                  ),
                  Positioned(
                    ...
                    child: Container(
                      ...
                      child: Row(children: [
                        ...
                        Text(_zoomBukti ? 'Perkecil' : 'Perbesar', style: const TextStyle(fontSize: 10, color: Colors.white)),
                      ]),
                    ),
                  ),
                ],
          ...
          Row(children: [
            ...
            const Text('Diupload oleh mitra: 15 Apr 2026, 10.32', style: TextStyle(fontSize: 10, color: AppColors.textSecondary)),
          ]),
            ...
            Row(children: [
              ...
              Expanded(
                child: ElevatedButton(
                  ...
                  child: const Text('Setujui Pembayaran', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w500)),
                ),
              ),
            ])
          else
...
class _SheetInfoCell extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      const SizedBox(height: 2),
      Text(value, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w600, color: highlight ? AppColors.textSuccess : AppColors.textPrimary)),
    ]);
  }
}
  ...
  Widget build(BuildContext context) {
    return Expanded(
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        const SizedBox(height: 2),
        Text(value, style: TextStyle(fontSize: 12, fontWeight: FontWeight.w500, color: valueColor ?? AppColors.textPrimary)),
      ]),
    );
  }
}
